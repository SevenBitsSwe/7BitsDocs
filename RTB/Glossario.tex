...
\newcommand\version{1.0.1} %aggiunta versione come variabile
...
\endhead
    \hline
    1.0.1 & 2025-03-21 & Alfredo Rubino & Uncas Peruzzi & Aggiunta dei nuovi termini derivanti dal documento Manuale Utente\\
    \hline
...
\hline
API Key & Una chiave di autenticazione utilizzata per accedere ai servizi API di Groq, che forniscono elaborazione ad alte prestazioni per modelli di intelligenza artificiale. Questa chiave è necessaria per autenticare le richieste e garantire un accesso sicuro alle risorse e alle funzionalità offerte dalla piattaforma Groq.\\
\hline
...
\hline
Flink & Apache Flink è un framework open-source per l'elaborazione di stream di dati in tempo reale. Nel nostro progetto, Flink viene utilizzato per processare i dati trasmessi tramite Kafka, applicando trasformazioni, aggregazioni e analisi in tempo reale per estrarre informazioni utili e garantire una gestione efficiente del flusso dati.\\
\hline
...
\endhead
\hline
Pannello & L'interfaccia grafica utilizzata per la visualizzazione e il monitoraggio dei dati elaborati. Questo può includere dashboard sviluppate con strumenti come Grafana, che permettono di rappresentare metriche di sistema, eventi in tempo reale e prestazioni del flusso dati.\\
\hline
