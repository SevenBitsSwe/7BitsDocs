...
\hline
0.4.0 & 2025-01-8 & Giovanni Cristellon &  & raggruppamento UC 7-12 in UC7\\
\hline
...
\subsubsection{\textbf{UC7 - Creazione annuncio}}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{UC7image.png}
    \caption{UC7 - Creazione annuncio}
    \label{fig:UC7}
\end{figure}
\label{UC7}
\begin{itemize}
    ...
    \item \textbf{Precondizioni:} 
        \begin{itemize}
          \item Il sistema è operativo e accessibile;
          \item Il Database è operativo e accessibile;
          \item Il sistema di Stream Processing è attivo e ha validato i dati che costantemente riceve in input.
        \end{itemize}
      \item \textbf{Postcondizioni:} \\
    \item \textbf{Scenario Principale:} 
        \begin{enumerate}
        \item Il sistema di stream processing invia una tupla contente l'identificativo univoco di un utente e dati posizionali dello stesso
        \item Il sistema verifica che il tempo trascorso dal ultimo annuncio non sia inferiore al cooldown minimo
        \item Il sistema ottiene i dati del utente e dei punti di ineresse prossimi dal database
        \item Il sistema genera una richiesta al servizio LLM
        \item Il servizio LLM risponde con un annuncio inerente agli interessi del utente ed i punti di interesse prossimi
        \item Il sistema inserisce l'annuncio nel database
    \end{enumerate}
    \item \textbf{Scenari alternativi:} 
        \begin{itemize}
        \item 2a: il tempo dal ultimo annuncio è inferiore al cooldown
            \begin{enumerate}
                la richiesta del servizio di stream processing viene ignorata
            \end{enumerate}
        \item 5a il sistema LLM non ha inserito "NO MATCH" nella risposta
            \begin{enumerate}
                la richiesta del servizio di stream processing viene ignorata
            \end{enumerate}
        \item 5b il sistema LLM genera una risposta mal formata o con elementi non ammessi
            \begin{enumerate}
                la richiesta del servizio di stream processing viene ignorata
                viene generato un log del evento nel database
            \end{enumerate}
        \end{itemize}
    \item \textbf{User-Story$_G$ associata:} Come sistema di Stream Processing voglio effettuare una richiesta per la creazione di un annuncio inerente agli interessi del utente e i punti di interesse prossimi ad egli
\end{itemize}
