...
\newcommand\version{0.5.3} %aggiunta versione come variabile
...
\section{Processi di supporto}
    ...
    La verifica è un processo essenziale che deve essere applicato a tutti gli altri processi per poterli considerare completati. Il suo scopo principale è quello di assicurarsi che i prodotti siano corretti ed aderiscano ai vincoli individuati ed elencati nel documento \textit{Piano di Qualifica}. Le attività di verifica sono assegnate ai verificatori, che si impegnano a far rispettare i vincoli di qualità, presenti nel \textit{Piano di Qualifica}, correggendo e/o segnalando quando sono presenti modifiche non conformi agli standard.
    \subsubsection{Strumenti}
    Gli strumenti utilizzati per agevolare il processo di verifica sono i seguenti:
    \paragraph{GitHub}
    GitHub fornisce un servizio di review all'interno del meccanismo di PR, permettendo al verificatore di visualizzare le ultime modifiche. Il verificatore una volta verificato il documento può inserire commenti specifici, con all'interno correzioni o migliorie da apportare, inviando una richiesta di modifica nella PR o approvare la PR e procedere con la verifica di altri prodotti presenti in repository in attesa di verifica. La review di GitHub consente all'intero team di poter tracciare le modifiche effettuate all'interno della repository.\\
    ...
    Effettuata sul prodotto software o sulla documentazione senza dover eseguire il sistema software. Serve ad accertare la conformità a regole adottate, l'assenza di difett e la presenza di proprietà desiderate, garantendo un livello ottimale di qualità. Le due principali tecniche sono Inspection e Walkthrough. Il gruppo preferisce l'approccio ad Inspection, così da ridurre il tempo di verifica e poter lasciare concentrare l'autore sulla stesura di altri documenti o codice.
    ...
    Il verificatore e l'autore del codice/documento da verificare discutono sulle modifiche da apportare. Questo processo richiede la presenza e la collaborazione di entrambi i ruoli.Viene effettuato un controllo ad ampio spettro dell'oggetto, senza avere un'idea precisa di cosa andare a verificare.
    

    \subsubsection{Analisi dinamica}
    Effettuata esclusivamente sul prodotto software tramite l'esecuzione effettiva di esso. L'analisi dinamica si divide in test. Ogni test è una esecuzione di un programma e provano il comportamento di singole parti di codice su un insieme finito di casi di prova.\\
    Questa procedura misura la qualità del prodotto e assicura che esegua correttamente il suo scopo. I test effettuati devono essere ripetibili nel tempo per assicurare funzionamento persistente durante tutto lo svolgimento del prodotto software.
    ...
    \begin{itemize}
    \item \textbf{Funzionale}: verificano che ogni unità esegue le funzione specificate nel design, facendo riferimento solo alla specifica input-output dell'oggetto di verifica;
    ...
    L'obiettivo di questo test è di accertarsi che l'intero sistema lavori come entità unica, verificando che tutte le singole componenti funzionino insieme. Viene eseguita al completamento dei test di integrazione.
    ...
    Consiste nella ripetizione selettiva delle tipologie dei test elencati precedentemente e devono essere eseguiti ogni volta che si effettuano modifiche al codice.

    \paragraph{Test di accettazione}
    Test da passare prima del rilascio del software. Serve a garantire che il prodotto finale soddisfi le aspettative del proponente e degli utenti finali.

    \paragraph{Classificazione dei test}
    I test vengono identificati in base alla loro tipologia e tramite un codice numerico, specifico all'interno della categoria. I test devono avere la seguente forma: \textbf{T[Tipologia Test][Codice]}\\
    Tipologia:
    \begin{itemize}
    \item \textbf{U}: Unità;
...
\newpage
\section{Standard ISO/IEC:25010 "System \& software quality models"}
Lo standard ISO/IEC:25010 appartiene alla famiglia di standard SQuaRE, ed è stato sviluppato e pubblicato nel 2011 congiuntamente 
dalla ISO e dalla IEC. Trattasi di un modello internazionale che definisce in maniera chiara un quadro di riferimento per la valutazione
e la misurazione della qualità di un prodotto software. Lo scopo è di garantire che il prodotto realizzato soddisfi i requisiti 
di utenti, di proponenti e di committenti garantendo al contempo sicurezza e funzionalità.
Lo standard non entra mai nel merito del "cosa" fa il software, ma definisce il "come" opera nel contesto della qualità.
    ...
    \end{itemize}
        ...
        \begin{itemize}
            \item Functional Completeness - il software deve mettere a disposizione tutte le funzionalità necessarie;
            \item Functional Correctness - il software deve eseguire le sue funzionalità correttamente e nel modo previsto;
            \item Functional Appropriateness - il software deve offrire funzionalità appropriate agli obiettivi;
        \end{itemize}
        ...
        \begin{itemize}
            \item Maturity - un software maturo è stato testato a fondo, pertanto molti bug sono stati individuati e risolti;
            \item Availability - misura la frequenza con la quale il software è "up-and-running";
            \item Fault Tolerance - rappresenta la capacità del software di funzionare anche in presenza di errori;
            \item Recoverability - rappresenta la capacita del software di recuperare da un failure riprendendo le sue funzioni;
        \end{itemize}

        \subsubsection{Usability}
        Misura quanto intuitivo, soddisfacente e facile da usare è un software. Si compone di cinque sotto-caratteristiche:
        \begin{itemize}
            \item Understandability - rappresenta la facilità con cui gli utenti possono capire le funzionalità offerte da un software;
            \item Learnability - misura quanto facilmente un nuovo utente può imparare a fare uso di un software;
            \item Operability - indica quanto controllo l'utente ha sul software stesso;
            \item Attractiveness - misura l'attrattiva estetica che l'interfaccia del software esercita sull'utente;
            \item Accessibility - indica la facilità di uso del sistema da parte di utenti con disabilità;
        \end{itemize}
        ...
        \begin{itemize}
            \item Time behavior - è indicatore di quanto velocemente un sistema può eseguire un compito o processare una richiesta;
            \item Resource utilization - misura quanto efficientemente un software utilizza le risorse fisiche dell'hardware sottostante;
            \item Capacity - misura la capacità del sistema di gestire determinati carichi di lavoro;
        \end{itemize}
        ...
        \begin{itemize}
            \item Modularity - indica il grado di suddivisione in componenti fra loro indipendenti del sistema;
            \item Reusability - indica la capacità di riutilizzare componenti software;
            \item Analyzability - rappresenta la facilità con cui è possibile diagnosticare problemi del software;
            \item Modifiability - indica la facilità con cui è possibile effettuare dei cambiamenti;
            \item Testability - rappresenta la facilità con cui è possibile testare il software modificato o prodotto;
        \end{itemize}
        ...
        \begin{itemize}
            \item Adaptability - rappresenta la capacità del sistema di adattarsi a diversi ambienti;
            \item Installability - indica la facilità di installazione del software;
            \item Replaceability - rappresenta la capacità del sistema di sostituire altri software;
        \end{itemize}
        ...
        \begin{itemize}
            \item Co-Existence - è indicatore della capacità del software di coesistere senza conflitti con altri sistemi;
            \item Interoperability - misura la capacità del software di scambiare informazioni con altri sistemi;
        \end{itemize}

        \subsubsection{Security}
        Indica la capacità di un prodotto software di proteggere informazioni o dati, così che tutti coloro che interagiscono con tale prodotto (sia persone che altri 
        software) di avere accesso alle informazioni che è loro consentito visualizzare secondo il livello di autorizzazione che possiedono. Include:
        \begin{itemize}
            \item Confidentiality - rappresenta la protezione dei dati da accessi non autorizzati;
            \item Integrity - rappresenta la prevenzione della modifica non autorizzata di dati;
            \item Authenticity - indica la verifica dell'identità degli utenti;
            \item Accountability - rappresenta il tracciamento delle azioni degli utenti;
            \item Non-repudiation - rappresenta la capacità di garantire che un'azione non possa essere negata;
        \end{itemize}
...
\begin{itemize}
    %\item \textbf{MPD04 - Code Coverage}: percentuale di codice testato.
    \item   \textbf{MPD05 - Statement Coverage}:
            \begin{itemize}
                \item   \textbf{Descrizione:} percentuale di istruzioni eseguite durante l'esecuzione dei test
                \item   \textbf{Formula:}
                        \[
                        Statement\ coverage(\%) = \frac{Numero\ di\ istruzioni\ eseguite}{Numero\ totale\ di\ istruzioni} \cdot 100
                        \]
            \end{itemize}
    \item   \textbf{MPD06 - Branch Coverage}: percentuale di rami decisionali testati.
            \begin{itemize}
                \item   \textbf{Descrizione:} percentuale di rami decisionali percorsi l'esecuzione dei test
                \item   \textbf{Formula:}
                        \[
                        Branch\ coverage(\%) = \frac{Numero\ di\ rami\ eseguiti}{Numero\ totale\ di\ rami} \cdot 100
                        \]
            \end{itemize}
    \item   \textbf{MPD07 - Condition coverage}:
            \begin{itemize}
                \item   \textbf{Descrizione:} percentuale del numero di condizioni booleane testate in entrambi i valori durante l'esecuzione dei test
                \item   \textbf{Formula:}
                        \[
                        Condition\ coverage(\%) = \frac{Numero\ di\ condizioni\ booleane\ testate\ in\ entrambi\ i\ valori}{Numero\ totale\ di\ condizioni\ booleane} \cdot 100
                        \]
            \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{MPD10 - Utilizzo Risorse}: efficienza nell'uso delle risorse.
    \item \textbf{MPD11 - Tempo medio di risposta}: .
\end{itemize}

\subsubsection{Usabilità}
\begin{itemize}
    \item \textbf{MPD12 - Facilità di utilizzo}: facilità con cui gli utenti possono utilizzare il sistema.
    \item \textbf{MPD13 - Tempo medio di apprendimento}: tempo necessario agli utenti per apprendere l'uso del sistema.
\end{itemize}

\subsubsection{Manutenibilità}
\begin{itemize}
    \item \textbf{MPD14 - Complessità ciclomatica}: misura della complessità del codice.
...
\paragraph{Fornitura}
\begin{itemize}
    \item \textbf{MPC07 - Estimated at Completion (EAC)}: stima il costo totale del progetto basandosi sul rendimento attuale:
    \[
    EAC = AC + ETC
    \]
    \item \textbf{MPC08 - Estimate to Complete (ETC)}: stima dei costi rimanenti per completare il progetto:
    \[
    ETC = BAC - EV
    \]
    \item \textbf{MPC09 - Actual Cost (AC)}: indica il costo effettivamente sostenuto per completare il lavoro fino a un certo momento.
    \item \textbf{MPC10 - Earned Value (EV)}: misura il valore del lavoro effettivamente completato rispetto al valore pianificato.
    \item \textbf{MPC11 - Planned Value (PV)}: rappresenta il valore pianificato del lavoro da completare in un determinato momento.
\end{itemize}

\paragraph{Codifica}
\begin{itemize}
    \item \textbf{MPC12 - Complessità ciclomatica}: misura della complessità del codice.
...
\begin{itemize}
    \item   \textbf{MPC18 - Indice Gulpease}:
            \begin{itemize}
                ...
                \item   \textbf{Formula:}
                        \[
                        G = 89 - \frac{(300 \cdot N_{frasi}) \cdot (10 \cdot N_{lettere})}{N_{parole}}
                        \]
                        Dove:
                        \begin{itemize}
                            \item $N_{frasi}$ = numero totale di frasi nel testo
                            \item $N_{lettere}$ = numero totale di lettere nel testo
                            \item $N_{parole}$ = numero totale di parole nel testo
                        \end{itemize}
            \end{itemize}
    \item   \textbf{MPC19 - Correttezza ortografica}:
            \begin{itemize}
                \item \textbf{Descrizione:} trattasi di un numero che rappresenta la quantità di errori ortografici all'interno del documento considerato
            \end{itemize}
\end{itemize}

\paragraph{Verifica}
\begin{itemize}
    \item \textbf{MPC20 - Code coverage}: percentuale di codice testato.
    \item \textbf{MPC21 - Statement coverage}: .
    \item \textbf{MPC22 - Branch coverage}: .
    \item \textbf{MPC23 - Condition coverage}: .
    \item \textbf{MPC24 - Passed test cases percentage}: percentuale di test superati.
\end{itemize}

\paragraph{Gestione della Qualità}
\begin{itemize}
    \item \textbf{MPC25 - Metriche di qualità soddisfatte}: grado di soddisfacimento delle metriche di qualità stabilite.
\end{itemize}

\subsubsection{Processi Organizzativi}
\paragraph{Gestione dei Processi}
\begin{itemize}
    \item \textbf{MPC26 - Rischi non previsti}: monitoraggio dei rischi non inclusi nelle stime.
    \item \textbf{MPC27 - Efficienza Temporale (ET)}: rapporto tra ore produttive ($O_p$) e ore totali ($O_o$):
    \[
    ET = \frac{O_p}{O_o}
    \]
\end{itemize}
